...
#
# The tree is at zero, so this gates rather than reports: `make docs-placement`
# reports without failing. It needs no toolchain -- awk over `git ls-files` -- so
# it runs on a bare checkout, before anything is built.
#
...
#
# Three comment lines in json/src end in `is` today -- json-parsers.ads:33,
# json-types.adb:44 and json-types.ads:402. None reports a finding, because no
# declaration follows the block in any of the three. json-types.adb:44 is the
# fragile one: one reflow of that prose puts `function To_Text` under it and
# produces a phantom finding. Recognise the shape before believing the report:
#   grep -nE '^[[:space:]]*--.*[[:space:]](is|declare|private|record)$' <file>
#
# Usage:
...
# Vendored or generated Ada we do not author, as a path regex. Empty: every
# tracked .ads/.adb here -- json/src, json/prove, tools/src, tests/src -- is
# authored in-tree. It stays as the one knob a port has to set.
readonly NOT_OURS=''
if [ -n "$NOT_OURS" ]; then
